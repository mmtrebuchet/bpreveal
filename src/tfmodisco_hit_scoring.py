...
def main(
    ...
    center_cut_size, min_ic, metacluster_ind,
    pattern_inds, input_length
):
    ...
    peak_table = pd.read_csv(
        ...
        usecols=[0, 1, 2],
        names=["peak_chrom", "peak_start", "peak_end"]
    )
    print(peak_table.head(n=1)["peak_start"].values)
    output_length = peak_table.head(n=1)["peak_end"].values[0] - peak_table.head(n=1)["peak_start"].values[0]
    logging.info("Output length: {0:d}".format(output_length))
    padding = (input_length - output_length) //2

    logging.info("Importing DeepSHAP scores and TF-MoDISco results...")
    hyp_scores, act_scores, one_hot_seqs, imp_coords = \
        import_shap_scores(
            shap_scores_path, center_cut_size=center_cut_size,
            remove_non_acgt=False)
    
    logging.info(one_hot_seqs.shape)
    ...
    logging.info("Loaded modisco results.")
    # Expand peaks to input length
    peak_table["peak_start"] = \
        peak_table["peak_start"] - padding
    ...
    logging.info("Matching up DeepSHAP coordinates and peak coordinates...")
    ...
    logging.info("Matched peaks to shap coordinates.")
    ...
    logging.info("Preparing the hit scorer...")
    ...
    logging.info("Starting hit scoring...")
    batch_size = 1024
    num_batches = int(np.ceil(len(act_scores_matched) / batch_size))
    rows = []

    for i in range(num_batches):
        logging.info("\tScoring batch %d/%d" % (i + 1, num_batches))
    ...
    logging.info("Cleaning up matches...")
    ...
    for i, row in match_table.iterrows():
        if row["score_start"] < 0 or row["score_end"] >= score_length:
            logging.warn("Hit at %s:%d-%d is outside of importance score range" % (
...
if __name__ == "__main__":
    ...
    minIc = config["min-ic"]
    metaclusterIndex = config["metacluster-index"]
    patternIndexes = config["pattern-indexes"]
    if(patternIndexes == "all"):
        #The main code expects this to be a list of ints or None.
        patternIndexes = None
    shapScoresPath = config["shap-scores-h5"]
    modiscoResultsPath = config["modisco-h5"]
    peakBedPath = config["peak-bed"]
