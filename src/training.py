...
def buildLosses(heads: dict) -> tuple[list, list]:
    """Given the output head specification (from the configuration JSON), build the losses.
    ...
    1. A list of all the profile losses followed by all the counts losses:
        ``profileLoss1, profileLoss2, ... profileLossN, countsLoss1, countsLoss2, ... countsLossN``
    2. A list of all the loss weights. Since the counts loss weight is included in the
        actual loss function, the weights that are given to the Keras training routine are
        all ones for the counts. The profile loss weights are taken straight from your json.
...
def trainModel(model: keras.Model, trainBatchGen: generators.H5BatchGenerator,
    ...
    :param heads: The heads from the configuration JSON, AFTER you have injected losses with
        :py:func:`buildLosses<bpreveal.training.buildLosses>`.
