...
def main(config):
    ...
    generator = pisa.BedGenerator(config["bed-file"], config["genome"], config["input-length"], config["output-length"])
    ...
    #For benchmarking, I've added a feature where you can dump a python profiling session to disk. You should probably
    ...
    batcher = pisa.PisaRunner(config["model-file"], config["head-id"], config["task-id"], 10, generator, writer, config["num-shuffles"], receptiveField, profileFname)
    batcher.run()
