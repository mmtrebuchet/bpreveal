...
def main(config):
    ...
    """
    hyp_scores, act_scores, input_seqs, _ = import_shap_scores(config["shap-h5"], config["center-cut-size"])
    ...
    if(config["verbosity"] in ["INFO", "DEBUG"]):
        modisco_verbose=True
    else:
        modisco_verbose=False

    # Construct workflow pipeline
    tfm_workflow = modisco.tfmodisco_workflow.workflow.TfModiscoWorkflow(
        sliding_window_size=config["sliding-window-size"],
        max_seqlets_per_metacluster=config["max-seqlets-per-metacluster"],
    	flank_size=config["flank-size"],
        target_seqlet_fdr=config["target-seqlet-fdr"],
        verbose = modisco_verbose,
    	seqlets_to_patterns_factory=modisco.tfmodisco_workflow.seqlets_to_patterns.TfModiscoSeqletsToPatternsFactory(
            embedder_factory=modisco.seqlet_embedding.advanced_gapped_kmer.AdvancedGappedKmerEmbedderFactory(),
    	    trim_to_window_size=config["sliding-window-size"],
            verbose=modisco_verbose,
            n_cores = config["num-cores"],
    	    initial_flank_to_add=config["flank-size"],
    	    final_min_cluster_size=config["final-min-cluster-size"]
    	)
    ...
    os.chdir(cwd)
    logging.info("Saving results to %s" % (config["output-dir"] + "/modisco.h5"))
    with h5py.File(config["output-dir"] + "/modisco.h5", "w") as f:
    ...
    for mc in range(metacluster_count):
        ...
        for idx,pattern in enumerate(patterns):
            ...
            logging.info("pattern idx {}".format(idx))
