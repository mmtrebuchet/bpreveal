...
class Hit: holds mapped coordinate and and quality information of each hit
...
def ppmToPwm(ppm: npt.NDArray, backgroundProbs: npt.NDArray) -> npt.NDArray[MOTIF_FLOAT_T]:
    ...
    :param backgroundProbs: is an ndarray of shape (4) representing
        probabilities to normalize information context over occuring
        frequencies
...
class Pattern:
    ...
    def loadCwm(self, modiscoFp: h5py.File, trimThreshold: float,
        ...
        :param trimThreshold: is a floating point number. The lower this is,
            the more flanking bases will be kept.
        :param padding: is a an integer. After trimming, pad each end of motif
            by this many flanking bases to reconstruct flank specificity.
        :param backgroundProbs: is an ndarray of shape (4) representing
            probabilities to normalize information context over occuring
            frequencies
        :return: Loads the cwm, ppm, pwm, pssm motif representations as well
            as metadata concerning the trimmed motif boundaries.
    ...
    def loadSeqlets(self, modiscoFp: h5py.File) -> None:
        ...
        :param modiscoFp: is a filepath referencing the modisco.h5 file 
            generated by tfmodiscolite
        :return: Loads the seqlet coordinates and associated quality information metadata
    ...
    def getCutoffs(self, quantileSeqMatch: float,
        ...
        :param quantileSeqMatch: is a float designating the minimum PSSM match quantile
            ...
            PSSM matche scores. If value is None instead of float, skip this theshold.
        :param quantileContribMatch: is a float designating the minimum CWM match quantile
            threshold that a mapped hit must meet based on the distribution of seqlet
            CWM matche scores. If value is None instead of float, skip this theshold.
...
class Hit:
    ...
    metaclusterName
        ...
        whether the sequence feature increases or decreases SHAP value
    strand
        Strand alignment of the hit coordiante
    sequence
    ...
    contribMatchScore
        Match score to the hit's motif CWM based on Jaccardian similarity
    seqMatchScore
...
class PatternScanner:
    """A class which provides functionality to scan a CWM/PSSM/other
