    ...
    def seqletInfoIterator(self):
        ...
        for i in range(self.numSeqlets):
            ...
            ret = {
                ...
                "end": self.seqletGenomicEnds[i],
                "short_name": self.shortName,
                "contrib_magnitude": self.seqletContribMagnitudes[i],
                "strand": "-" if self.seqletRevcomps[i] else "+",
                "metacluster_name": self.metaclusterName,
                "pattern_name": self.patternName,
                "sequence": self.seqletSequences[i],
                "region_index": self.seqletIndexes[i],
                "seq_match": self.seqletSeqMatches[i],
                "contrib_match": self.seqletContribMatches[i]
            }
            yield ret
