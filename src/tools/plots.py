...
def plotLogo(values: PRED_AR_T, width: float, ax, colors = 'seq') -> None:
    ...
    def _drawLetter(text: str, left: float, right: float, bottom: float, top: float,
        ...
        patch = PathPatch(charPath, facecolor=color)
        ax.add_patch(patch)
...
def plotPisa(pisaDats, cutMiddle, cutLengthX, cutLengthY, receptiveField,
    ...
    #                    receptiveField//2+cutStartX:receptiveField//2+cutStartX + cutLengthX]
    plotMat = shearMat[cutStartY:cutStartY + cutLengthY,
    ...
    l,b,w,h = bbox
    axPisa = fig.add_axes([l, b+h/8, w*7/8, 7*h/8])

    axPisa.imshow(plotMat, vmin=-colorSpan, vmax=colorSpan, extent=extent, cmap='RdBu_r', aspect='auto', interpolation='nearest')
    ...
    axPisa.set_yticks([axStartY, axStopY],
                      [str(genomeWindowStart+cutStartY),  str(genomeWindowStart+cutStartY + cutLengthY)],
                      fontsize=5)
    ...
    if motifScanBedFname is not None:
        ...
        annotations = []
        for line in bedFp:
            if line.chrom == genomeWindowChrom and line.end > lowerBound and line.start < upperBound:
        ...
        axAnnot = fig.add_axes([l, b+5*h/8, w*7/8, 3*h/8])
        ...
        for annot in sorted(annotations, key=lambda x: x[0][0]):
            ...
            if offset < -1:
                continue
            axAnnot.fill([aleft,aleft,aright,aright], [offset, boxHeight + offset,
                                     boxHeight + offset, offset],
